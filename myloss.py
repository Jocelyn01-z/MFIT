...
class Loss(nn.Module):
    ...
    def weighted_BCE_loss_pseudo(self,target_pre,sub_target,inc_L_ind,epoch,reduction='mean'):
        ...
        trance_inc_L_ind=1-inc_L_ind#表示缺失标签引导矩阵
        # 缺失位置填充1为伪标签
        # res111=res1.mul(trance_inc_L_ind)    
        
        res11=res1.mul(inc_L_ind)     #不考虑缺失的情况   
        res12=res1.mul(1-inc_L_ind)  #考虑缺失的情况
        
        meanloss = torch.sum(res11, dim=1, keepdim=True)/torch.sum(inc_L_ind, dim=1, keepdim=True)
        pso_inc_L_ind= (res12 < meanloss).float()#采用0表示缺失的，把它们全部作为可信的为标签，若损失小于均值，证明是有效的
        pso_inc_L_ind=pso_inc_L_ind.mul(1-inc_L_ind)# 值为1表示用0填充是有效的
